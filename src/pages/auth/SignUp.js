...
import { Col, Alert, Row, Card, Form, Button, Spinner, Image } from 'react-bootstrap';
import axios from 'axios';
// import media files
import Logo from 'assets/images/brand/logo/logo-icon.svg';
export default function SignUp() {
    const axiosRequest = axios.create({
        baseURL: "https://qaamuus.academy"
    });
    ...
    async function userJoinHandleSubmit(e) {
        ...
        if (userData.get('password') == userData.get('comfirmPassword')) {
            setFormError('')
            await axiosRequest.get('api/checkUserExistEmailAndUsername/' + userData.get('username') + '/' + userData.get('email') + '/').then(async (userResp) => {
                ...
                if (userResp.data.isExist == true) {
                    ...
                    setFormIsLoading(false)
                    try {
                        await axiosRequest.post('/api/user-create/', userData).then(async (dataResp) => {
                            console.log(dataResp.data)
                            console.log(dataResp.data.data.id)
                            userData.append('user',parseInt(dataResp.data.data.id))
                            await axiosRequest.post('/api/userProfile-create/', userData).then((userProfileResp) => {
                                setFormIsLoading(false)
                    ...
                    <Card>
                        <Card.Body className="p-6">
                            ...
                            <Image src={Logo} className="mb-4" alt="" />
                                ...
                                {userRegistred ? (
                                   <Alert variant="success" >User was succesfully registred</Alert>
                            ...
                            <Form onSubmit={userJoinHandleSubmit} >
                                <Row>
                                    <Col lg={3} md={3} className="mb-3">
                                        <Image className="mb-4" width="150px"  alt="" src='https://www.commisceo-global.com/images/easyblog_articles/1783/jordanian-man.jpg' />
                                    </Col>
                                    <Col lg={9} md={9} className="mb-3">
                                        <Form.Group size='sm' controlId="formFile" className="mb-3">
                                            <Form.Label size='sm'>User Image</Form.Label>
                                            <Form.Control required name="profileImage" size='sm' type="file" accept="image/*" />
                                        </Form.Group>
                                    </Col>
                                    ...
                                    <Col lg={12} md={12} className="mb-3">
                                        ...
                                        <Form.Check type="checkbox" id="check-api-checkbox">
                                            <Form.Check.Input type="checkbox" onClick={togglePassword} />
                                            <Form.Check.Label>
                                                SHOW PASSWORD
