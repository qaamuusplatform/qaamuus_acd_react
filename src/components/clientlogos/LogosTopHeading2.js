...
import cmnData from "services/baseUrl";
const LogosTopHeading2 = ({ logos, title, limit }) => {
  ...
  const LogoImage = ({ logo }) => {
    return (
      <Col md={4} lg={2} sm={6} xs={6} className="mb-4 mb-lg-0">
        <Image height={60} width={100} src={logo} alt="" />
      </Col>
    );
  };

  // .map((logo, index) => <LogoImage key={index} logo={cmnData.baseUrl+logo.image} />);
  const LogosList = () => {
    if (limit > 0) {
      return (logos || [])
        .slice(0, limit)
  ...
  // <LogoImage key={index} logo={cmnData.baseUrl+logo.image} />
  return (
